...
* 按外键映射： 给外键添加一个唯一约束。 一个表中用many-to-one 加一个外键，并加上唯一约束(unique=true)；另外一个表中加入one-to-one约束。
	...
	-  查询：
		+ 1.默认情况下，对关联属性使用懒加载
		+ 2.同时提前关闭session也会出现lazyInitialization Exception
		+ 3.在one-to-one中(即没有外键的一端)需要添加property-ref属性来指定使用被关联实体主键以外的字段作为关联字段，否则hibernate会默认使用主键来进行管理，从而导致一些关联对应错误
		+  4.在查询没有外键实体对象时，使用的是左外链接，一并查询其关联对象并进行初始化。(了解即可)
