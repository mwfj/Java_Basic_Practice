...
<form action="updateProdServlet" method="post" 
	...
	<table>
    	...
    	<tr>
    		<td>商品类型</td>
    		<td>
    			<input type="checkbox" name="hotProds" value="1" ${product.isHot==1? 'checked':''} readonly="readonly"/>热卖
				<input type="checkbox" name="newProds" value="1" ${product.isNew==1? 'checked':''} readonly="readonly"/>新品
				<input type="checkbox" name="saleProds" value="1" ${product.isSale==1? 'checked':''} readonly="readonly"/>特价
			</td>
    	</tr>
    	<tr>
    		<td>
    			<input type="button" id="modif" value="修改" onclick="modifProd()"/>
    			<input type="button" id="save" value="保存" onclick="saveProd()" style="display:none"/>
    		</td>
    		<td><a href="getProdPagerServlet">返回</a>&nbsp;<font color="red">${message }</font></td>
    	</tr>
    </table>
 </form>
</center>
